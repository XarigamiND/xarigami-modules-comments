...
/**
 * Comments API
 * Defines inmportant constants for use throught the renderer
 */
...
function comments_renderer_array_markdepths_bypid(&$comments_list)
{
    if (empty($comments_list) || !count($comments_list)) {
        throw new BadParameterException('comments_list');
    }
...
function comments_renderer_array_prune_excessdepth($args)
{
    ...
    if (TRUE == xarModGetVar('comments','dochildcount')) {
        ...
        foreach ($comment_list as $node) {
            switch($sortby) {
                case _COM_SORTBY_TOPIC:
                    $key = preg_replace('/\\:/i', ' ', $node['xar_title']);
                    break;
...
function comments_renderer_wrap_words(&$str, $chars)
{
    ...
    if (xarModGetVar('comments','wrap') && !xarModIsHooked('bbcode','comments',0)){
